...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/03 21:40:17 by jmacgyve          #+#    #+#             */
/*   Updated: 2019/10/21 23:18:01 by jmacgyve         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	itoa_base_printf(uintmax_t n, int b, t_printf *p)
{
	...
	while (tmp && ++p->printed)
		tmp /= b;
	(p->f & F_ZERO) ? p->accuracy = p->min_length : 0;
	ext = (p->printed >= p->accuracy) ? 0 : 1;
	p->printed = MAX(p->accuracy, p->printed);
