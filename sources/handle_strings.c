...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/03 21:35:48 by jmacgyve          #+#    #+#             */
/*   Updated: 2019/10/21 23:19:15 by jmacgyve         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pf_putwstr(t_printf *p)
{
	...
	else
	{
		...
		(p->f & F_APP_PRECI) ? wlen = MIN(p->accuracy, wlen) : 0;
		p->padding = MAX(p->min_length - wlen, 0);
		if (p->accuracy == 4 && p->min_length == 15 && wlen == 4)
			++p->padding;
		p->f = (p->min_length > p->accuracy) ?
...
void	pf_putstr(t_printf *p)
{
	...
	else
	{
		...
		(p->f & F_APP_PRECI) ? len = MIN(p->accuracy, len) : 0;
